...
class Blob {
  ...
  render() {
    ...
    ctx.fillStyle = `rgba(250, 188, 63, 0.5)`; // Light yellow with transparency
    ctx.fill();
    ...
    if (this.mousePos) {
      this.points.forEach(point => {
        point.updateForMouseInteraction(this.mousePos, this.radius + 20); // Update radius to be 20px larger
      });
    }
  ...
  get position() {
    return this._position || { x: 0.9, y: 0.1 };
  }
...
const BlobCanvas3 = () => {
  ...
  const updateBlobProperties = () => {
    if (blobRef.current) {
      ...
      if (width <= 768) {
       blobRef.current.radius=0;
       blobRef.current.position = { x:-2, y: 0.3 };
      } else {
        blobRef.current.radius = 200; // Default radius for larger screens
        blobRef.current.position = { x: 0.9, y: 0.3 }; // Default position for larger screens
      }
      blobRef.current.render(); // Re-render the blob with updated properties
    }
  };
  ...
  useEffect(() => {
    ...
    blob.canvas = canvas;
    blob.init();
    updateBlobProperties(); // Set initial properties based on screen size
    blobRef.current = blob;

    const resizeCanvas = () => {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
      updateBlobProperties(); // Update properties on resize
    };
